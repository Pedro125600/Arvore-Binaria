...
namespace att1_
{
    ...
    class ArvoreBinaria
    {
        ...
        public int[] CaminharPos()
        {
            int count = 0;
            int[] v = new int[ContarElementos(raiz)];
            CaminharPos(raiz, v, ref count);
            return v;
        }
        private void CaminharPos(No i, int[] v, ref int count)
        {
            if (i != null)
            {
                CaminharPos(i.Esq,  v, ref count);
                CaminharPos(i.Dir,v, ref  count);
                v[count] = i.Elemento;
                count++;
            }
        }
